...
/**
 * Copy inequal items to a temp collection, then delete from and reinsert into
 * collection 1.
 *
 * @param {mongodb.Collection} coll  the collection to update
 * @param {mongodb.Collection} tmpColl  the temp. collection to use
 * @param {Array} inequalItems  all the inequal items of collection 1 and 2
 * @param {String} attr  the attribute to sync
 * @param {Function} cb  first parameter is error or null, second parameter the
 *                       number of updated documents.
 */
function _strategyTemp(coll, tmpColl, inequalItems, attr, cb) {
  var itemsFrom1 = inequalItems.map(function(items) { return items.item1; });
  ...
  tmpColl.drop(function(err) {
    ...
    tmpColl.insert(itemsFrom1, { w: 1 }, function(err, inserted) {
      ...
      coll.remove({ _id: { $in: itemsFrom1.map(function(i) { return i._id; }) } }, { w: 1 }, function(err, removed) {
        ...
        inequalItems.forEach(function(items) {
 ...
 *                       number of updated documents.
 */
function _strategyDirect(coll, inequalItems, attr, cb) {
  var error;
  var updated = 0;
  async.eachSeries(inequalItems, function(items, cb2) {
...
function syncAttr(coll1, coll2, tmpColl, attr, opts, cb) {
  ...
  if (opts.includeAttrs) { throw new TypeError('use of opts.includeAttrs is prohibited'); }
  if (opts.excludeAttrs) { throw new TypeError('use of opts.excludeAttrs is prohibited'); }

  if (!Object.keys(opts.matchAttrs).length) {
    throw new Error('provide opts.matchAttrs');
  }

  if (opts.matchAttrs[attr]) { throw new Error('matching on attribute that is synced for comparison makes no sense'); }

  opts.includeAttrs = {};
  opts.includeAttrs[attr] = true; // set attr for comparison
  ...
  compare(coll1, coll2, opts, function(err, stats) {
    ...
    if (DEBUG) { console.log('inequal', stats.inequal.length); }

    if (!stats.inequal.length) { cb(null, 0); return; }

    if (attr === '_id') {
      if (DEBUG) { console.log('temp strategy'); }
      _strategyTemp(coll1, tmpColl, stats.inequal, attr, cb);
    } else {
      if (DEBUG) { console.log('direct strategy'); }
      _strategyDirect(coll1, stats.inequal, attr, cb);
    }
  });
}
