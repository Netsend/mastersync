...
VersionedCollection._branchHeads = function _branchHeads(items, includeDeleted, includeConflicted) {
  ...
  items.forEach(function(item) {
    ...
    if (!item._id._c || includeConflicted) {
        ...
        newThis._findLCAs(item, lastLocalItem, function(err, lcas) {
          ...
          that._virtualCollection.findOne(selector, { sort: { '_id._i': -1 }, comment: '_ensureLocalPerspective2_3' }, function(err, lca) {
            ...
            newThis._merge(item, lca, function(err, merged) {
              ...
              if (!merged[1]._m3) {
                error = new Error('local perspective of new item misses _m3');
                if (!that._hide) { console.error(that.databaseName, that.collectionName, '_ensureLocalPerspective2 4', error, JSON.stringify(merged)); }
                return cb2(error);
              }
              ...
              // set _m3 on new merged item
              merged[1]._m3._ack = false;
              merged[1]._m3._op = new Timestamp(0, 0);
...
VersionedCollection.prototype._merge = function _merge(objX, objY, cb) {
  ...
  that._findLCAs(itemX, itemY, function(err, lcas) {
    ...
    // TODO: ignore versions that are in conflict
