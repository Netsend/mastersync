...
VersionedCollection.prototype.determineRemoteOffset = function determineRemoteOffset(pe, cb) {
  if (this.debug) { console.log(this.databaseName, this.collectionName, 'determineRemoteOffset'); }
 ...
 * @param {Number, default 5} [free]  the percentage of free space that should be available
 * @param {Function} cb  The first parameter will contain either an Error object or
 *                       null.
 */
...
VersionedCollection.prototype._ensureSamePerspective = function _ensureSamePerspective(items, cb) {
  ...
  if (that.debug) { console.log(that.databaseName, that.collectionName, '_ensureSamePerspective', JSON.stringify(items)); }
...
VersionedCollection.prototype._ensureM3 = function _ensureM3(items, cb) {
  ...
  if (that.debug) { console.log(that.databaseName, that.collectionName, '_ensureM3', JSON.stringify(items)); }
...
VersionedCollection.prototype._checkAncestry = function _checkAncestry(items, cb) {
  ...
  if (that.debug) { console.log(that.databaseName, that.collectionName, '_checkAncestry', JSON.stringify(items)); }
  ...
  items.forEach(function(itemNcb) {
    ...
    if (!item._id._pa.length) {
      ...
      if (length && DAGs[item._id._id][length - 1]._id._d) {
        ...
        if (that.debug) { console.log(that.databaseName, that.collectionName, '_checkAncestry connected new root to previous (deleted) item', JSON.stringify(item)); }
      } else if (length) {
        ...
        if (!that._hide) { console.error(that.databaseName, that.collectionName, '_checkAncestry', error, JSON.stringify(item)); }
        throw error;
      }
    }
...
VersionedCollection.prototype._markConflicts = function _markConflicts(items, cb) {
  ...
  if (that.debug) { console.log(that.databaseName, that.collectionName, '_markConflicts', JSON.stringify(items)); }
  ...
  if (that.debug) {
    console.log(that.databaseName, that.collectionName, '_markConflicts nonConflictedHeads', JSON.stringify(nonConflictedHeads));
    console.log(that.databaseName, that.collectionName, '_markConflicts conflictedHeads', JSON.stringify(conflictedHeads));
  }
...
VersionedCollection.prototype._ensureOneHead = function _ensureOneHead(items, cb) {
  ...
  if (that.debug) { console.log(that.databaseName, that.collectionName, '_ensureOneHead', JSON.stringify(items)); }

  var newHeads = VersionedCollection._branchHeads(items, true, false);

  if (that.debug) { console.log(that.databaseName, that.collectionName, '_ensureOneHead new heads', JSON.stringify(newHeads)); }

  that._markConflicts(newHeads, function (err, nonConflictedNewHeads, conflictedNewHeads) {
    if (err) { console.log('err', err); cb(err); return; }
    if (nonConflictedNewHeads.length !== 1) {
      var error = new Error('not exactly one head');
      if (!that._hide) {
        console.error(that.databaseName, that.collectionName, '_ensureOneHead not exactly one head', error, nonConflictedNewHeads.length, JSON.stringify(items));
      }
      cb(error);
      return;
    }
...
VersionedCollection.prototype._ensureVirtualCollection = function _ensureVirtualCollection(items, cb){
  ...
  if (that.debug) { console.log(that.databaseName, that.collectionName, '_ensureVirtualCollection', JSON.stringify(items)); }

  try {
    this._virtualCollection = new VirtualCollection(this._snapshotCollection, items);
    cb(null);
  } catch (err) {
    cb(err);
    return;
...
VersionedCollection.prototype._checkParentsInVirtualCollection = function _checkParentsInVirtualCollection(items, cb){
  ...
  async.eachSeries(items, function(item, cb2) {
    if (that.debug) { console.log(that.databaseName, that.collectionName, '_checkParentsInVirtualCollection', JSON.stringify(item)); }
    ...
    async.each(item._id._pa, function(p, cb3) {
      ...
      that._virtualCollection.findOne(selector, { sort: { '_id._i': -1 }, comment: '_checkParentsInVirtualCollection' }, function(err, theParent) {
        if (err) {
          if (!that._hide) { console.error(that.databaseName, that.collectionName, '_checkParentsInVirtualCollection', err, JSON.stringify(item)); }
          return cb3(err);
        }
        if (!theParent) {
          var error = new Error('parent not found');
          if (!that._hide) { console.error(that.databaseName, that.collectionName, '_checkParentsInVirtualCollection', error, p, JSON.stringify(item), JSON.stringify(selector)); }
          return cb3(error);
        }
        cb3();
      });
...
VersionedCollection.prototype._mergeNewHeads = function _mergeNewHeads(items, newRoots, cb) {
  ...
  async.eachSeries(items, function(head, cb2) {
    if (that.debug) { console.log(that.databaseName, that.collectionName, '_mergeNewHeads', JSON.stringify(head._id)); }
    ...
    that._snapshotCollection.findOne(selector, { sort: { '_id._i': -1 }, comment: '_mergeNewHeads' }, function(err, localHead) {
      ...
      if (newRoots[head._id._id] && !head._id._pa.length && localHead && localHead._id._d) {
        ...
        if (that.debug) { console.log(that.databaseName, that.collectionName, '_mergeNewHeads connected new root to previous (deleted) item', JSON.stringify(head)); }
      }
      ...
      if (newRoots[head._id._id] && !head._id._pa.length && localHead && !localHead._id._pa.length && localHead._id._v !== head._id._v) {
        ...
        if (!that._hide) { console.error(that.databaseName, that.collectionName, '_mergeNewHeads', head._id._id, error, JSON.stringify(head), JSON.stringify(localHead)); }
      ...
      if (!localHead) {
        if (that.debug) { console.log(that.databaseName, that.collectionName, '_mergeNewHeads no local head found', JSON.stringify(head)); }
        return cb2();
      ...
      newThis._merge(localHead, head, function(err, merged) {
        ...
        if (err) {
          if (that.debug) {
            console.log(that.databaseName, that.collectionName, '_mergeNewHeads merge error', head._id._id, err, JSON.stringify(localHead), JSON.stringify(head));
          }
          // skip merge conflicts
          if (err.message === 'merge conflict') {
            // TODO: mark as conflict?
            if (that.debug) {
              console.log(that.databaseName, that.collectionName, '_mergeNewHeads merge conflict marked', head._id._id, err, JSON.stringify(localHead), JSON.stringify(head));
            }
            head._m3._c = true;
            return cb2();
          }
        ...
        if (merged[0]._id._v) {
          ...
          if (that.debug) { console.log(that.databaseName, that.collectionName, '_mergeNewHeads ff without merge'); }
          return cb2();
        }

        if (that.debug) { console.log(that.databaseName, that.collectionName, '_mergeNewHeads merge created', JSON.stringify(merged[0]._id)); }

        // this is a genuine merge, version and add this new item
        merged[0]._m3._ack = false;
        merged[0]._m3._op = new Timestamp(0, 0);
...
VersionedCollection.prototype._ensureIntoSnapshot = function _ensureIntoSnapshot(perspective, localItems, remoteItems, cb) {
  ...
  try {
    ...
    if (that.debug) {
      console.log(that.databaseName, that.collectionName, '_ensureIntoSnapshot perspective', JSON.stringify(perspective));
      console.log(that.databaseName, that.collectionName, '_ensureIntoSnapshot remote items', JSON.stringify(remoteItems));
      console.log(that.databaseName, that.collectionName, '_ensureIntoSnapshot local items', JSON.stringify(localItems));
    }
    ...
    if (that.debug) {
      console.log(that.databaseName, that.collectionName, '_ensureIntoSnapshot new local heads', JSON.stringify(newLocalHeads));
      console.log(that.databaseName, that.collectionName, '_ensureIntoSnapshot remote items length', remoteItems.length);
      console.log(that.databaseName, that.collectionName, '_ensureIntoSnapshot local items length', localItems.length);
      console.log(that.databaseName, that.collectionName, '_ensureIntoSnapshot new local heads length', newLocalHeads.length);
    }
    ...
    async.eachSeries(localItems, function(item, cb2) {
      that._getNextIncrement(function(err, i) {
        if (err) {
          if (!that._hide) { console.error(that.databaseName, that.collectionName, '_ensureIntoSnapshot add seq', err, i, item); }
          return cb2(err);
        }
      ...
      async.eachSeries(allNew, function(newItem, cb3) {
        ...
        if (that.debug) { console.log(that.databaseName, that.collectionName, '_ensureIntoSnapshot exists?', JSON.stringify(selector)); }

        that._snapshotCollection.findOne(selector, { comment: '_ensureIntoSnapshot' }, function(err, exists) {
          if (err) {
            if (!that._hide) { console.error(that.databaseName, that.collectionName, '_ensureIntoSnapshot CRITICAL', newItem._id, err); }
            return cb3(err);
          }
          if (that.debug && exists) { console.log(that.databaseName, that.collectionName, '_ensureIntoSnapshot already exists', exists, newItem); }

          if (!exists) {
            that._snapshotCollection.insert(newItem, {w: 1, comment: '_insertIntoSnapshot' }, function(err, inserted) {
              if (err) {
                if (!that._hide) { console.error(that.databaseName, that.collectionName, '_insertIntoSnapshot CRITICAL', err); }
                return cb3(err);
              }
              if (that.debug) { console.log(that.databaseName, that.collectionName, '_insertIntoSnapshot inserted', inserted); }
              cb3();
            });
          } else {
            cb3();
...
VersionedCollection.prototype._insertIntoSnapshot = function _insertIntoSnapshot(perspective, localItems, remoteItems, cb) {
  ...
  try {
    ...
    if (that.debug) {
      console.log(that.databaseName, that.collectionName, '_insertIntoSnapshot perspective', JSON.stringify(perspective));
      console.log(that.databaseName, that.collectionName, '_insertIntoSnapshot remote items', JSON.stringify(remoteItems));
      console.log(that.databaseName, that.collectionName, '_insertIntoSnapshot local items', JSON.stringify(localItems));
    }
    ...
    if (that.debug) {
      console.log(that.databaseName, that.collectionName, '_insertIntoSnapshot new local heads', JSON.stringify(newLocalHeads));
      console.log(that.databaseName, that.collectionName, '_insertIntoSnapshot remote items length', remoteItems.length);
      console.log(that.databaseName, that.collectionName, '_insertIntoSnapshot local items length', localItems.length);
      console.log(that.databaseName, that.collectionName, '_insertIntoSnapshot new local heads length', newLocalHeads.length);
    }
    ...
    async.eachSeries(localItems, function(item, cb2) {
      that._getNextIncrement(function(err, i) {
        if (err) {
          if (!that._hide) { console.error(that.databaseName, that.collectionName, '_insertIntoSnapshot add seq', err, i, item); }
          return cb2(err);
        }
...
VersionedCollection.prototype._syncLocalHeadsWithCollection = function _syncLocalHeadsWithCollection(newLocalHeads, cb) {
  ...
  if (that.debug) { console.log(that.databaseName, that.collectionName, '_syncLocalHeadsWithCollection', JSON.stringify(newLocalHeads)); }

  async.eachSeries(newLocalHeads, function(head, cb2) {
    that._syncDAGItemWithCollection(head, function(err) {
      if (err) {
        if (!that._hide) { console.error(that.databaseName, that.collectionName, '_syncLocalHeadsWithCollection CRITICAL', err); }
        return cb2(err);
      }

      if (that.debug) { console.log(that.databaseName, that.collectionName, '_syncLocalHeadsWithCollection synced', JSON.stringify(head)); }
        ...
        async.eachSeries(Object.keys(DAGs), function(id, cb2) {
          ...
          that._ensureVirtualCollection(items, function(err) {
            ...
            that._checkParentsInVirtualCollection(items, function(err) {
              ...
              that._ensureLocalPerspective2(items, function(err, newOrExistingLocalItems) {
                if (err) { return cb2(err); }

                if (that.debug) { console.log(that.databaseName, that.collectionName, '_ensureLocalPerspective2 new local items', JSON.stringify(newOrExistingLocalItems)); }

                if (newOrExistingLocalItems.length) {
                  if (perspective === that.localPerspective) {
                    var error = new Error('local duplicates created');
                    if (!that._hide) { console.error(that.databaseName, that.collectionName, '_ensureAllInDAG', error); }
                    cb2(error);
                    return;
                  }
 ...
 *                       null.
 *
 * Note: ms always inserts 500 versions at a time while keeping 1 head for each item,
 * so when the system crashes only the last 500 should be corrected
 */
VersionedCollection.prototype.fixConsistency = function fixConsistency(pe, cb) {
  var that = this;

  if (that.debug) { console.log(that.databaseName, that.collectionName, 'fixConsistency perspective', JSON.stringify(pe)); }

  // select last 500 items from snapshot collection for specified pe
  var selector = { '_id._pe': pe };

  this._snapshotCollection.find(selector, { sort: { $natural: -1 }, limit: 500, comment: 'fixConsistency' }, function(err, dags) {
    // add all to DAG
 ...
 *                       non-local input.
 */
VersionedCollection.prototype._ensureLocalPerspective2 = function _ensureLocalPerspective2(items, cb) {
  var that = this;
  ...
  try {
    ...
    items.forEach(function(item) {
      if (perspective !== item._id._pe) {
        if (!that._hide) { console.error(that.databaseName, that.collectionName, '_ensureLocalPerspective2 perspective mismatch', perspective, item._id._pe); }
        throw new TypeError('perspective mismatch');
      }
    });
  } catch(err) {
  ...
  async.eachSeries(items, function(item, cb2) {
    if (that.debug) { console.log(that.databaseName, that.collectionName, '_ensureLocalPerspective2 1 new item', JSON.stringify(item._id)); }
    ...
    var opts = { sort: { $natural: -1 }, sortIndex: '_id._i', comment: '_ensureLocalPerspective2' };
    that._virtualCollection.findOne(selector, opts, function(err, lastLocalItem) {
      if (err) { cb2(err); return; }

      if (that.debug) { console.log(that.databaseName, that.collectionName, '_ensureLocalPerspective2 1 last local item', JSON.stringify(lastLocalItem)); }
      ...
      newThis._isAncestorOf(item._id._v, lastLocalItem, function(err, isAncestor) {
        ...
        if (isAncestor) {
          if (that.debug) { console.log(that.databaseName, that.collectionName, '_ensureLocalPerspective2 2 is ancestor of last local item', item._id._v); }
        ...
        newThis._findLCAs(item, lastLocalItem, function(err, lcas) {
          ...
          if (lcas.length < 1) {
            ...
            if (!that._hide) { console.error(that.databaseName, that.collectionName, item._id._id, '_ensureLocalPerspective2 2', error, description); }
          ...
          if (lcas.length > 1) {
            ...
            if (!that._hide) { console.error(that.databaseName, that.collectionName, '_ensureLocalPerspective2 2', error, description); }
            return cb2(error);
          }

          if (that.debug) { console.log(that.databaseName, that.collectionName, '_ensureLocalPerspective2 2 lca', JSON.stringify(lcas[0])); }

          // fetch the lca with local perspective
          selector = { '_id._id': item._id._id, '_id._v': lcas[0], '_id._pe': that.localPerspective };
          that._virtualCollection.findOne(selector, { sort: { '_id._i': -1 }, comment: '_ensureLocalPerspective2_3' }, function(err, lca) {
            if (err) { return cb2(err); }
            if (!lca) {
              error = new Error('lca with local perspective not found');
              if (!that._hide) { console.error(that.databaseName, that.collectionName, '_ensureLocalPerspective2 3', error + description); }
              return cb2(error);
            }

            if (that.debug) { console.log(that.databaseName, that.collectionName, '_ensureLocalPerspective2 3 fetched lca', JSON.stringify(lca._id)); }

            // create a perspective bound item by merged fast-forward
            newThis._merge(item, lca, function(err, merged) {
              if (err) {
                if (!that._hide) { console.error(that.databaseName, that.collectionName, '_ensureLocalPerspective2 4 merge', err); }
                return cb2(err);
              }

              if (!merged[1]._id._v) {
                error = new Error('new version created while expecting fast-forward by merge');
                if (!that._hide) { console.error(that.databaseName, that.collectionName, '_ensureLocalPerspective2 4', error, JSON.stringify(merged)); }
                return cb2(error);
              }

              if (merged[1]._m3) {
                if (that.debug) {
                  console.log(that.databaseName, that.collectionName, '_ensureLocalPerspective2 4 local perspective already exists', JSON.stringify(merged[1]));
                }
                cb2();
                return;
              }
              ...
              if (that.debug) {
                console.log(that.databaseName, that.collectionName, '_ensureLocalPerspective2 4 merge created', JSON.stringify(merged[1]));
              }
    ...
    if (err) {
      if (!that._hide) { console.error(that.databaseName, that.collectionName, '_ensureLocalPerspective2', err); }
      return cb(err);
    }
