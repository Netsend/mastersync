...
/**
 * Check if a DAG has only one head
 *
...
VersionedCollection.prototype._ensureOneHead = function _ensureOneHead(items, cb) {
  ...
  try {
    var newHeads = VersionedCollection._branchHeads(items, true);
    ...
    if (newHeads.length !== 1) {
      ...
      if (!that._hide) {
        console.error(that.databaseName, that.collectionName, '_ensureOneHead not exactly one head', error, newHeads.length, JSON.stringify(items));
      }
      cb(error);
      return;
    }
 ...
 * - addAllToDag 500 batch size times 3, because all have local pe and a merge potentially
 * Note2: _lo can never be in conflict
 */
VersionedCollection.prototype.fixConsistency = function fixConsistency(pe, cb) {
  ...
  this._snapshotCollection.find(selector, { sort: { $natural: -1 }, limit: 500, comment: 'determineRemoteOffset' }, function(err, dags) {
    // add all to DAG
