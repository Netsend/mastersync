...
/**
 * Ensure a virtual collection exists or one is created
 *
...
/**
 * Check if every parent of every new item exists in the virtual DAG.
 *
...
VersionedCollection.prototype._insertIntoSnapshot = function _insertIntoSnapshot(perspective, localItems, remoteItems, cb) {
  ...
  try {
    ...
    async.eachSeries(localItems, function(item, cb2) {
      ...
      that._snapshotCollection.insert(allNew, {w: 1, comment: '_insertIntoSnapshot' }, function(err) {
        if (err) {
    ...
    that._ensureM3(newItems, function(err) {
      ...
      that._checkAncestry(newItems, function(err, allItems, DAGs, newRoots) {
        ...
        async.eachSeries(Object.keys(DAGs), function(id, cb2) {
          ...
          that._ensureOneHead(items, function(err) {
            ...
            that._ensureVirtualCollection(items, function(err) {
              ...
              that._checkParentsInVirtualCollection(items, function(err) {
                ...
                that._ensureLocalPerspective(items, function(err, newLocalItems) {
                  ...
                  if (that.debug) { console.log(that.databaseName, that.collectionName, '_ensureLocalPerspective new local items', JSON.stringify(newLocalItems)); }
