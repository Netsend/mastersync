 ...
 *   usersColl {String, default users}  collection that contains all user accounts
 *   replicationDb {String}  in which database replication configs are stored. By
 *                 default, the database in the auth request is used.
 *   replicationColl {String, default replication}  collection that contains all
 *                   replication configs
...
function VersionedSystem(oplogColl, opts) {
  ...
  if (typeof opts !== 'undefined') {
    ...
    if (typeof opts.usersColl !== 'undefined' && typeof opts.usersColl !== 'string') { throw new TypeError('opts.usersColl must be a string'); }
    ...
    if (typeof opts.replicationColl !== 'undefined' && typeof opts.replicationColl !== 'string') { throw new TypeError('opts.replicationColl must be a string'); }
  }
  ...
  this._usersCollName = this._options.usersColl || 'users';

  this._replicationDb = this._options.replicationDb;
  this._replicationCollName = this._options.replicationColl || 'replication';
...
VersionedSystem.prototype.initVCs = function initVCs(vcs, follow, cb) {
  ...
  async.eachSeries(Object.keys(vcs), function(dbName, cb2) {
    async.eachSeries(Object.keys(vcs[dbName]), function(collectionName, cb3) {
      ...
      if (that._vcs[ns]) {
        error = new Error('vc already initialized');
      ...
      if (that._oplogReaders[ns]) {
        error = new Error('vc already has an oplog reader associated');
        if (!that._hide) { console.error('vs vc error', ns, error); }
        cb3(error);
        return;
      }
      ...
      that._startVC(vceCfg, function(err, vc, or) {
...
VersionedSystem.prototype.listen = function listen(user, newRoot, preauthCfg, cb) {
  ...
  if (!Object.keys(this._vcs).length) { throw new TypeError('run initVCs first'); }
  ...
  function connErrorHandler(conn, err) {
    console.error('vs listen connection error', conn.address(), err);

    try {
      conn.write('invalid auth request\n');
      conn.destroy();
    } catch(err) {
      console.error('vs listen connection write or disconnect error', err);
    }
  }
  ...
  function handleMessage(req, conn) {
    ...
    that._verifyAuthRequest(req, function(err, replCfg) {
      ...
      if (that.debug) { console.log('vs push request forwared', pushReq); }
...
/**
 * Stop pre-auth server, oplog reader and vc instances.
 *
 * @param {Function} cb  First parameter will be an Error object or null.
 */
...
VersionedSystem.prototype._startVC = function _startVC(config, cb) {
  ...
  vc.stdout.setEncoding('utf8');
  vc.stderr.setEncoding('utf8');

  vc.stderr.pipe(process.stderr);

  if (this.debug) {
    vc.stdout.pipe(process.stdout);
  }

  vc.once('exit', function(code, sig) {
...
VersionedSystem.prototype._verifyAuthRequest = function _verifyAuthRequest(req, cb) {
  ...
  var usersColl = usersDb.collection(this._usersCollName);
  ...
  var replicationColl = replicationDb.collection(this._replicationCollName);
